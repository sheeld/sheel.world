...
function webfonts($custom_fontset, $is_ce) {
	...
	if($is_ce) {
		...
		// css classes
		$classes['standard'] = '.wysiwyg h1, .wysiwyg h2, .wysiwyg h3, .wysiwyg h4, .wysiwyg h5, .wysiwyg h6, .wysiwyg-ce h1, .wysiwyg-ce h2, .wysiwyg-ce h3, .wysiwyg-ce h4, .wysiwyg-ce h5, .wysiwyg-ce h6';
	}
	...
	if($custom_fontset) {
		...
		if($type === 'service') {
			...
			$font_output .= '<style data-fontset-id="ce-fontset" type="text/css">';
		...
		function get_fonts($weight, $is_body_font, $is_ce, $classes) {
			...
			if($options['font_name']) {
				...
				if($is_body_font) {
					...
					// if also headings font weight is bold, add it
					if($weight === get_field('so_headings_font_weight', 'options')) {
						$headings_font_classes = ', ' . $classes['standard'];
					}
					$font_css = '.' . $weight . ', strong, b' . $headings_font_classes . ' { font-family: ' . $font_name . ', ' . $category . '; ' . $font_style  . ' ' . $font_weight . ' }';	
		...
		$font_output .= '<link data-fontset-id="ce-fontset" href="https://fonts.googleapis.com/css?family=Open+Sans:300,300italic,400,400italic,600,600italic,700,700italic" rel="stylesheet" type="text/css">';
		$font_output .= '<link data-fontset-id="ce-fontset" href="https://fonts.googleapis.com/css?family=PT+Serif:400,400italic" rel="stylesheet" type="text/css">';
		$font_output .= '<style id="ce-fontset" type="text/css">';
